...
from data_loader import create_temp_directories, prepare_logo_dataset
...
os.makedirs('model_checkpoint_logo', exist_ok=True)

def prepare_datasets(train_dirs: List[str], val_dirs: List[str], test_dirs: List[str]) -> Tuple:
    """
    ...
    Args:
        ...
        val_dirs (List[str]): List of directories for validation data.
        test_dirs (List[str]): List of directories for test data. 
    Returns:
        Tuple: Training, validation, and test iterators along with temp directory.
    """
    logging.info(f"Preparing datasets... Train: {train_dirs}, Validation: {val_dirs}, Test: {test_dirs}")
    train_temp_dir, val_temp_dir, test_temp_dir = create_temp_directories(train_dirs, val_dirs, test_dirs)
    ...
    logging.info(f"Total training images found: {train_it.samples}")
    ...
    logging.info(f"Total validation images found: {val_it.samples}")
    ...
    logging.info(f"Total test images found: {test_it.samples}")
...
def plot_training_curves(history: tf.keras.callbacks.History, output_dir: str, identifier: str) -> None:
    ...
    Args:
        ...
        identifier (str): Identifier for the current experiment.
    """
...
def run_experiment(model: tf.keras.Model, train_it, val_it, test_it, checkpoint_dir: str, identifier: str) -> tf.keras.callbacks.History:
    ...
    report_txt = classification_report(y_true, y_pred, target_names=['Cool', 'Neutral', 'Warm'])
...
if __name__ == "__main__":
    base_dir = 'all_data'
    checkpoint_dir = "model_checkpoint_logo"
    ...
    logo_datasets = prepare_logo_dataset(base_dir, sub_group_definitions)
    for main_group_index, (train_participants, validation_participants, test_participant) in enumerate(logo_datasets, start=1):
        identifier = f"MainGroup{main_group_index}_{'_'.join(test_participant)}"
        train_data = [os.path.join(base_dir, participant, 'data') for participant in train_participants]
        val_data = [os.path.join(base_dir, participant, 'data') for participant in validation_participants]
        test_data = [os.path.join(base_dir, participant, 'data') for participant in test_participant]
